...
pub fn collect_nodes(path: &Path) -> Vec<Node> {
...
mod tests {
    ...
    fn create_file_in_directory(directory: &Path, basename: &str) -> String {
    ...
    fn collect_nodes_with_branch_containing_leaf() {
        ...
        let expected = [
            Node::Branch(
                branch.clone(),
                [
                    Node::Leaf(create_file_in_directory(branch_path, "leaf"))
                ].to_vec(),
            )
        ].to_vec();
